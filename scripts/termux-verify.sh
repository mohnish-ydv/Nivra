...
DEST="${NIVRA_D5_TEST_DIR:-$HOME/nivra-d5-verification}"
...
case "$DEST" in
  "$HOME"/*) ;;
  *)
    echo "FAIL: NIVRA_D5_TEST_DIR must stay inside Termux home: $HOME"
    exit 1
    ;;
esac

echo "Copying D5 to Termux internal storage:"
echo "  $DEST"
rm -rf "$DEST"
mkdir -p "$DEST"
cp -R "$ROOT/." "$DEST/"
